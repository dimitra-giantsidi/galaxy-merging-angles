...
def signal_gen(name, num, box, age, mh, D, year, plot=True):
    ...
    path = f"../{box}_{num}_data/survey.{name}.0.h5"
    ...
    # PARSEC:
    iso = ascii.read(f"../signal_files/parsec_{age}{mh}_iso.txt")
    order = np.argsort(iso['rmag'])
    mag2 = iso['rmag'][order]
    mag1 = iso['gmag'][order]

    # PARSEC:
    lf = ascii.read(f"../matched_filter_testing/iso_lf_signals/iso_lf/parsec_{age}{mh}_lf.txt")
    ...
    ascii.write(tab,f'../matched_filter_testing/iso_lf_signals/signal_files/parsec_{int(D/1000)}_{age}{mh}_signal_cmd_gr.txt',overwrite=True)
...
def bg_dwarf_combo(pdist,name,year,D,c1,c2,plotdir,dwarfcatpath,bgcatpath,edgelength=5,plot=True,save=True):
    ...
    if save : # and not center_cut:
        ...
        table_final.write(f'../../matched_filter_testing/catalogs/{name}_{c1}{c2}_formf_bg{int(edgelength)}deg.fits',format='fits',overwrite=True)
